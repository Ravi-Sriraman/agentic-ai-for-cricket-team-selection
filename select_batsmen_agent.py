...
def create_pick_top_batsmen_agent():
    ...
    system_prompt = """
        ...
        6. return the top 10 result in JSON format with fields player_name, average, strike_rate, performance_index.
        """.format(dialect=db.dialect,top_k=5)

    llm = init_chat_model(model="gpt-4o", model_provider="openai")
    toolkit = SQLDatabaseToolkit(db=db, llm=llm)
    tools = toolkit.get_tools()
    return create_agent(model=llm, tools=tools, system_prompt=system_prompt, name="pick_top_batsmen_agent")


def main():
    load_dotenv()
    agent = create_batsmen_agent()
