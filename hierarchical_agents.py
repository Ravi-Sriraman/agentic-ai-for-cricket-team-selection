...
@tool
def write_document(
        content: Annotated[str, "Text content to be written to document."],
        file_name: Annotated[str, "File path to save the document to."],
) -> Annotated[str, "Path of the saved document document ."]:
...
def edit_document(
        ...
        inserts: Annotated[Dict[int, str], "Dictionary where key is the line number (1-index) and value is the text to be inserted at that line."],
) -> Annotated[str, "Path of the editored document file ."]:
    """Edit the document by inserting provided text at specified line numbers."""
    with (WORKING_DIRECTORY / file_name).open("w") as file:
       lines =  file.readlines()
